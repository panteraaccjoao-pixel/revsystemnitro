...
let faqHtml = '<section class="relative py-20 overflow-hidden"><div class="container mx-auto px-4"><div class="text-center mb-12" style="opacity:0;transform:translateY(50px)"><span class="text-xs uppercase tracking-[0.25em] text-primary font-medium">Dúvidas Frequentes</span><h2 class="text-3xl md:text-4xl font-extrabold text-foreground mt-3">FAQ</h2></div><div class="max-w-3xl mx-auto space-y-3">';

faqQuestions.forEach(item => {
    faqHtml += \<div class="" style="opacity:0;transform:translateY(50px)"><div class="rounded-xl border border-border/40 bg-card/30 overflow-hidden transition-colors hover:border-border/60"><button type="button" class="faq-button w-full flex items-center justify-between px-5 py-4 text-left" aria-expanded="false"><span class="text-sm font-semibold text-foreground pr-4">\</span><div class="shrink-0 transition-transform duration-300 faq-icon"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-plus w-4 h-4 text-primary"><path d="M5 12h14"></path><path d="M12 5v14"></path></svg></div></button><div class="faq-content overflow-hidden transition-all duration-300" style="height: 0px;"><div class="px-5 pb-5 pt-1 text-sm text-muted-foreground border-t border-border/10 mt-1">\</div></div></div></div>\;
});

faqHtml += '</div></div></section>';

// Find the section to replace
const sectionRegex = /<section class="relative py-20 overflow-hidden"><div class="container mx-auto px-4"><div class="text-center mb-12" style="opacity:0;transform:translateY\(50px\)"><span class="text-xs uppercase tracking-\[0\.25em\] text-primary font-medium">Dúvidas Frequentes<\/span><h2 class="text-3xl md:text-4xl font-extrabold text-foreground mt-3">FAQ<\/h2><\/div><div class="max-w-3xl mx-auto space-y-3">.*?<\/section>/s;

content = content.replace(sectionRegex, faqHtml);
